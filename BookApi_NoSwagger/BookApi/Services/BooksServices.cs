...
{
    public class BooksServices : CategoryServices
    {
        private readonly IMongoCollection<Book> _books;
        public BooksServices(IBookstoreDatabaseSettings settings, ICategoryBookDatabaseSettings category) : base(category)
        {
            var client = new MongoClient(settings.ConnectionString);
            var database = client.GetDatabase(settings.DatabaseName);
            _books = database.GetCollection<Book>(settings.BooksCollectionName);
            //base._category = database.GetCollection<CategoryBook>(category.CategoryCollectionName);
        }
        ...
        public async Task<List<BookDetails>> GetListBookDetails()
        {
            ...
            foreach (Book book in _books.Find(book => true).ToList())
            {
                ...
                var categoryBook = await base._category.Find(c => c.Id == book.Category).FirstOrDefaultAsync();
